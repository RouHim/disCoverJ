...
public class SearxCoverSearchEngine implements CoverSearchEngine {
  ...
  private static final String API_REQUEST =
    "search?categories=images&format=json&q=";
  ...
  private static List<ImageFile> startSearch(
    String query,
    String searxInstance
  ) {
    Optional<JSONObject> jsonFromUrl = getJsonFromUrl(
      searxInstance + API_REQUEST + query
    );
    ...
    Stream<ImageFile> base64Stream = searchResultData
      ...
      .filter(
        dataString ->
          dataString.startsWith("data") && dataString.contains("base64")
      )
  ...
  public Optional<String> checkInstance(String searxInstanceUrl) {
    try {
      JSONObject result = new JSONObject(
        IOUtils.toString(
          URI.create(searxInstanceUrl + API_REQUEST + "test"),
          UTF_8
        )
      );
      return result.has("results")
        ? Optional.empty()
        : Optional.of("Api returned no results");
    } catch (Exception e) {
