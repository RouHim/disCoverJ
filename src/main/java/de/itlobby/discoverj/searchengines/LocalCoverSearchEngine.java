...
public class LocalCoverSearchEngine implements CoverSearchEngine {
  ...
  public List<ImageFile> search(AudioWrapper audioWrapper) {
    ...
    if (
      ...
      !customFolderPath.isEmpty() &&
      new File(customFolderPath).exists()
    ) {
  ...
  private List<ImageFile> getCoverImagesFromOtherAudioFiles(
    ...
    List<LocalMatchInfo> filesInFolder = getFilesInFolder(
    ...
    List<ImageFile> foundImages = filesInFolder
      ...
      .filter(otherFile ->
        matchesCriteria(currentMatchInfos, otherFile, config)
      )
  ...
  private boolean matchesCriteria(
    LocalMatchInfo currentFile,
    LocalMatchInfo otherFile,
    AppConfig config
  ) {
    boolean matches =
      !config.isLocalMatchAlbum() ||
      currentFile.album().equals(otherFile.album());

    if (
      config.isLocalMatchAlbumArtist() &&
      !currentFile.albumArtist().equals(otherFile.albumArtist())
    ) {
      matches = false;
    }

    if (
      config.isLocalMatchYear() && !currentFile.year().equals(otherFile.year())
    ) {
      matches = false;
    }
  ...
  private List<ImageFile> getCoverFromCustomFolder(AudioWrapper audioWrapper) {
    ...
    return FileUtils.listFiles(customDir, VALID_IMAGE_FILE_EXTENSION, false)
      .stream()
      .filter(imageFile ->
        imageFile.getName().toLowerCase().contains(coverFileName.toLowerCase())
      )
      .parallel()
  ...
  private Comparator<ImageFile> imageSizeComparator() {
    return (x, y) ->
      Integer.compare(x.height() * x.width(), y.height() * y.width()) * -1;
  }
