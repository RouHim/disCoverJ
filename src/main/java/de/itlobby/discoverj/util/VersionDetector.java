...
public class VersionDetector {
    ...
    public static void determineCurrentVersion() {
        try {
            Enumeration<URL> manifestFiles = Thread.currentThread().getContextClassLoader()
                    .getResources(MANIFEST_PATH);

            List<Attributes> attributes =
                    Collections.list(manifestFiles)
                            .stream()
            ...
            attributes.stream()
                    ...
                    .findFirst()
                    .ifPresentOrElse(version -> Settings.getInstance().setVersion(new Version(version)),
