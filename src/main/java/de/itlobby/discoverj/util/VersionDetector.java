...
public class VersionDetector {
  ...
  public static void determineCurrentVersion() {
    try {
      Enumeration<URL> manifestFiles = Thread.currentThread()
      ...
      List<Attributes> attributes = Collections.list(manifestFiles)
        ...
        .filter(manifestAttributes ->
          manifestAttributes.containsValue(Main.class.getName())
        )
        .toList();
      ...
      attributes
        ...
        .map(manifestAttributes ->
          manifestAttributes.getValue(DISCOVERJ_VERSION)
        )
        .findFirst()
        .ifPresentOrElse(
      ...
      ViewManager.getInstance().setPrimaryTitle(
        "disCoverJ " + Settings.getInstance().getVersion().toString()
      );
    } catch (Exception e) {
