...
public class AudioUtil {
  ...
  public static final String[] VALID_IMAGE_FILE_EXTENSION = {
    "jpg",
    "jpeg",
    "bmp",
    "png",
  ...
  public static final String[] VALID_AUDIO_FILE_EXTENSION = Arrays.stream(
    SupportedFileFormat.values()
  )
    .map(SupportedFileFormat::getFilesuffix)
    .filter(suffix -> !suffix.isEmpty())
    .toArray(String[]::new);
  static final Set<String> VALID_AUDIO_FILE_EXTENSION_LIST = Arrays.stream(
  ...
  public static boolean haveCover(AudioFile audioFile) {
    ...
    return (
      ...
      tag.getFirstArtwork().getBinaryData() != null
    );
  }
   ...
   */
  public static Optional<Image> getCover(
   ...
   */
  private static Optional<String> readMaybeFieldValue(
    AudioFile audioFile,
    FieldKey fieldKey
  ) {
    if (audioFile == null) {
  ...
  public static List<String> getArtists(AudioFile audioFile) {
    ...
    return Arrays.stream(
      readFieldValue(audioFile, FieldKey.ARTIST).split(artistSeparators)
    )
      .map(String::trim)
      .toList();
  }
  ...
  public static Optional<String> getMusicbrainzReleaseId(
    AudioWrapper audioWrapper
  ) {
    AudioFile audioFile = AudioUtil.readAudioFileSafe(
  ...
  public static Optional<Integer> getDiscogsReleaseId(AudioFile audioFile) {
    return readCustomFieldValue(audioFile, DISCOGS_RELEASE_ID).map(
      Integer::valueOf
    );
  }

  private static Optional<String> readCustomFieldValue(
    AudioFile audioFile,
    String customField
  ) {
    return StreamSupport.stream(
      Spliterators.spliteratorUnknownSize(
        audioFile.getTag().getFields(),
        Spliterator.ORDERED
      ),
      false
    )
  ...
  public static void saveCoverToAudioFile(
    AudioFile audioFile,
    BufferedImage cover
  ) {
    if (cover == null) {
  ...
  private static void writeAudioTag(AudioFile audioFile, BufferedImage cover)
  ...
  public static boolean hasSameFolderAndAlbumAsLast(
    ...
    try {
      ...
      if (
        !StringUtil.isNullOrEmpty(currentAlbum) &&
        !StringUtil.isNullOrEmpty(lastAlbum)
      ) {
  ...
  public static boolean isAudioFile(File file) {
    return VALID_AUDIO_FILE_EXTENSION_LIST.contains(
      StringUtil.getFileExtension(file).toLowerCase()
    );
  }
  ...
  public static String getImageResolutionString(Image cover) {
    return ImageUtil.createImageResolutionString(
      cover.getWidth(),
      cover.getHeight()
    );
  }
