...
public class ImageUtil {
  ...
  public static java.util.List<ImageView> createSearchEnginePictures(
    ...
    for (SearchEngine searchEngine : Settings.getInstance()
      .getConfig()
      .getSearchEngineList()) {
      ImageView imageView = new ImageView(
        new Image(searchEngine.getType().getLogoPath())
      );
      ...
      if (canEdit) {
        ...
        imageView.setOnMouseClicked(event ->
          toogleDisabled(imageView, searchEngine)
        );
      }
  ...
  public static void toogleDisabled(
    ImageView imageView,
  ...
  public static void setSearchEngineImageViewEffect(
    ImageView imageView,
    SearchEngine searchEngine
  ) {
  ...
  private static void initDragAndDrop(ImageView imageView, FlowPane flowPane) {
    ...
    imageView.setOnDragOver(event -> {
      if (
        ...
        !imageView.getId().equals(event.getDragboard().getString())
      ) {
    ...
    imageView.setOnDragDropped(event -> {
      ...
      if (db.hasString()) {
        ImageView src = (ImageView) SystemUtil.getChildrenById(
          flowPane,
          db.getString()
        );
        ...
        flowPane
          .getChildren()
          .add(flowPane.getChildren().indexOf(imageView), src);
  ...
  public static String createImageResolutionString(
    double width,
    double height
  ) {
  ...
  public static BufferedImage removeTopBottomBlackBorders(BufferedImage input) {
    ...
    try {
      ...
      int newHeight =
        input.getHeight() -
        (firstFilledLine) -
        (input.getHeight() - (lastFilledLine + 1));
  ...
  public static BufferedImage removeLeftRightBlackBorders(BufferedImage input) {
    ...
    try {
      ...
      int newWidth =
        input.getWidth() -
        (firstFilledColumn) -
        (input.getWidth() - (lastFilledColumn + 1));
  ...
  public static WritableImage toFXImage(
    BufferedImage image,
    int targetWidth,
    int targetHeight
  ) {
  ...
  public static Optional<Image> getFxImageFromBytes(
    byte[] data,
    int width,
    int height
  ) {
  ...
  public static Optional<Image> getFxImageFromBytes(byte[] data) {
    ...
    try {
      ...
      return Optional.ofNullable(
        toFXImage(rgbImage, rgbImage.getWidth(), rgbImage.getHeight())
      );
    } catch (Exception e) {
      log.error(e.getMessage(), e);
    }
