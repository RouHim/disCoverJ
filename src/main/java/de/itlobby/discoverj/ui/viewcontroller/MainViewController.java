...
public class MainViewController
  implements
    ViewController, MultipleSelectionListener, ParentKeyDeletedListener {
  ...
  public void activateSearchState(
    EventHandler<ActionEvent> stopEventHandler,
    int audioFilesCount
  ) {
  ...
  public void showAudioInfo(AudioWrapper audioWrapper, boolean showCover) {
    ...
    Platform.runLater(() -> {
      String fileSize = StringUtil.sizeToHumanReadable(
        audioWrapper.getFileLength()
      );
      txtFilename.setText(
        String.format("%s (%s)", audioWrapper.getFileName(), fileSize)
      );
    ...
    Thread.ofVirtual().start(() -> {
      Optional<Image> maybeCover = AudioUtil.getCover(
        audioWrapper.getFilePath()
      );
      Platform.runLater(() -> {
        if (maybeCover.isPresent()) {
          imgCurrentCover.setImage(maybeCover.get());
          txtCurrentAudioCoverRes.setText(
            AudioUtil.getImageResolutionString(maybeCover.get())
          );
        } else {
  ...
  private void initContextMenu() {
    ...
    alCmItemSearchSearXImages.setOnAction(e ->
      initialService.searchOnSearXImages()
    );
    ...
    alCmItemSearchGoogleImages.setOnAction(e ->
      initialService.searchOnGoogleImages()
    );
    ...
    alCmItemRemoveWithCover.setOnAction(e ->
      initialService.clearWithCoverListEntries()
    );
    ...
    alCmItemRemoveSelected.setOnAction(e ->
      initialService.clearSelectedEntries()
    );
    ...
    lwAudioList.addEventHandler(MouseEvent.MOUSE_CLICKED, e -> {
      if (
        ...
        !lwAudioList.getChildren().isEmpty()
      ) {
  ...
  private void registerKeyListener() {
    SelectionService selectionService = ServiceLocator.get(
    ...
    rootLayout.addEventHandler(KeyEvent.KEY_RELEASED, event -> {
      ...
      if (
        new KeyCodeCombination(KeyCode.A, KeyCombination.CONTROL_ANY).match(
          event
        )
      ) {
      ...
      if (
        new KeyCodeCombination(KeyCode.O, KeyCombination.CONTROL_ANY).match(
          event
        )
      ) {
      ...
      if (
        new KeyCodeCombination(KeyCode.HOME, KeyCombination.SHIFT_ANY).match(
          event
        )
      ) {
      ...
      if (
        new KeyCodeCombination(KeyCode.END, KeyCombination.SHIFT_ANY).match(
          event
        )
      ) {
  ...
  private void scrollToLastSelected() {
    AudioListEntry lastSelected = ServiceLocator.get(
      SelectionService.class
    ).getLastSelected();
  ...
  private void createBinding() {
    audioDetailsLayout
      .visibleProperty()
      .bind(txtFilename.textProperty().isNotEmpty());

    btnRemoveCover
      .visibleProperty()
      .bind(imgCurrentCover.imageProperty().isNotNull());
    btnCopyCoverToClipBrd
      .visibleProperty()
      .bind(imgCurrentCover.imageProperty().isNotNull());
    btnOpenGoogleImageSearch
  ...
  public void activateActionButton(
    ...
    Platform.runLater(() -> {
      ...
      AwesomeHelper.createCircleAnimation(
        btnActionCircle,
        btnActionCircleIcon,
        0,
        1,
  ...
  public void showBusyIndicator(String title, ActionListener cancelListener) {
    Platform.runLater(() -> {
      ...
      progressScanIndicator.setStyle(
        "-fx-accent: #FF6F00; -fx-progress-color: #FF6F00;"
      );
  ...
  public void setTotalAudioCountToLoad(int totalAudioCount) {
    Platform.runLater(() -> {
      ...
      Text fixPreText = new Text(
        LanguageUtil.getString("key.mainview.txtTotalMp3s") + " "
      );
      ...
      txtAudioToLoad
        .getChildren()
        .add(new Text(String.format(" / %s", totalAudioCount)));
    });
  }
  ...
  public void highlightInList(Integer audioWrapperId) {
    Platform.runLater(() -> {
      int i = lwAudioList
        .getChildren()
        .indexOf(getAudioListEntry(audioWrapperId));
  ...
  public void highlightRangeInList(AudioListEntry from, AudioListEntry to) {
    Platform.runLater(() -> {
      ...
      ServiceLocator.get(SelectionService.class).setSelectedEntries(
        selectedEntries
      );
    });
  }
  ...
  public void setNewCoverToListItem(
    AudioWrapper currentAudio,
    ImageFile newCover
  ) {
    Platform.runLater(() -> {
  ...
  public void createSingleLineAnimation(
    Image coverImage,
    AudioListEntry entry
  ) {
    Platform.runLater(() -> {
  ...
  public void scrollToNodeInList(AudioWrapper currentAudio) {
    Platform.runLater(() -> {
      ...
      ServiceLocator.get(SelectionService.class).addSelection(
        getAudioListEntry(currentAudio.getId())
      );
    });
  }
  ...
  public void onMultipleSelectionStared() {
    Platform.runLater(() -> {
      ...
      Parent layoutFromView = viewManager.createLayoutFromView(
        Views.MULTI_SELECTION_LAYOUT_VIEW
      );
      MultiselectionLayoutViewController viewController =
        viewManager.getViewController(Views.MULTI_SELECTION_LAYOUT_VIEW);
      viewController.setClearCoverText(
        LanguageUtil.getString("remove.all.cover.from.selected.audio.files")
      );
