...
public class InitialService implements Service {
  ...
  public void openInitialOpenDialog() {
    ...
    OpenFileViewController viewController =
      ViewManager.getInstance().getViewController(Views.DROP_FILE_VIEW);

    viewController.lblIntroduction.setText(
      LanguageUtil.getString("select.mp3s.here")
    );
    ...
    viewController.rootLayout.addEventHandler(MouseEvent.MOUSE_CLICKED, event ->
      findFolder()
    );

    LightBoxService lightBoxService = ServiceLocator.get(LightBoxService.class);
    lightBoxService.showDialog(
      LanguageUtil.getString("select.music"),
      root,
      null,
      null,
      false,
    ...
    viewController.btnExitDialog.setOnAction(event ->
  ...
  public void findFolder() {
    ...
    File lastPath = new File(
      config.getLastFolderPath() == null ? "" : config.getLastFolderPath()
    );
  ...
  public void beginScanFiles(File... musicObjects) {
    ...
    viewController.showBusyIndicator(
      LanguageUtil.getString("InitialController.loadingMp3List"),
      null
    );
  ...
  public void showCurrentCoverDetailed() {
    AudioListEntry audioListEntry = ServiceLocator.get(
    ...
    String title = MessageFormat.format(
      LanguageUtil.getString("cover.von.0"),
      audioWrapper.getDisplayValue()
    );
    ...
    CoverDetailViewController vc = ViewManager.getInstance().getViewController(
      Views.COVER_DETAIL_VIEW
    );
  ...
  private Parent createDetailCoverLayout() {
    return ViewManager.getInstance().createLayoutFromView(
      Views.COVER_DETAIL_VIEW
    );
  }
  ...
  public void removeAllSelectedCover() {
    ...
    List<AudioListEntry> selectedEntries =
      selectionService.getSelectedEntries();
          ...
          getMainViewController()
            ...
            .filter(x ->
              selectedEntries
                .stream()
                .anyMatch(y ->
                  y.getWrapper().getId().equals(x.getWrapper().getId())
                )
            )
            .forEach(AudioListEntry::removeCover);
        })
      ...
      audioWrapper
        ...
        .ifPresent(image ->
          Thread.ofVirtual().start(() ->
            new ImageClipboardUtil().setImage(image)
          )
        );
    }
  }
  ...
  public void pasteCoverFromClipBrd() {
    Thread.ofVirtual().start(() -> {
      ...
      Optional<AudioFile> audioFile = AudioUtil.readAudioFile(
  ...
  public void removeSelectedEntries() {
    ...
    if (!selectedEntries.isEmpty()) {
      DataHolder.getInstance().removeResultEntries(
        selectedEntries.stream().map(AudioListEntry::getWrapper).toList()
      );

      getMainViewController()
        .lwAudioList.getChildren()
        .removeAll(selectedEntries);
      ServiceLocator.get(SelectionService.class).clearAll();
    }
  }
  ...
  public void clearSelectedEntries() {
    List<Integer> selectedIds = ServiceLocator.get(SelectionService.class)
      .getSelectedEntries()
      .stream()
  ...
  public void searchOnSearXImages() {
    ...
    if (lastSelected != null) {
      String googleSearchPattern = Settings.getInstance()
        .getConfig()
        .getGoogleSearchPattern();
      String rawQuery = SearchQueryUtil.createSearchQueryFromPattern(
        lastSelected.getWrapper(),
        googleSearchPattern
      );
      ...
      Thread.ofPlatform().start(() -> {
        ...
        SystemUtil.browseUrl(
          String.format(url + "/search?q=%s&category_images", query)
        );
      });
  ...
  public void searchOnGoogleImages() {
    ...
    if (lastSelected != null) {
      ...
      SystemUtil.browseUrl(
        String.format("https://www.google.com/search?q=%s&tbm=isch", query)
      );
      openSelectImageDialog();
    }
  }
  ...
  private void openSelectImageDialog() {
    ...
    viewController.lblIntroduction.setText(
      LanguageUtil.getString("select.image.here")
    );
    viewController.lblIntroduction.getStyleClass().add("drop-zone-text");
    ...
    LightBoxService lightBoxService = ServiceLocator.get(LightBoxService.class);
    lightBoxService.showDialog(
      LanguageUtil.getString("select.cover"),
      root,
      null,
      null,
      false,
  ...
  private void findImage() {
    File imageFile = SystemUtil.getImageFileFromFileChooser(
      LanguageUtil.getString("select.cover")
    );
