...
public class SettingsService implements Service {
  ...
  public void initialize() {
    ...
    FlowPane searchEngineLayout =
      viewController.flowPaneSettingsSearchMachineOrder;

    AppConfig config = Settings.getInstance().getConfig();
    viewController.chkOverwriteCover.setSelected(config.isOverwriteCover());
    viewController.chkOverwriteOnlyHigher.setSelected(
      config.isOverwriteOnlyHigher()
    );
    viewController.txtMinCoverSize.setText(
      String.valueOf(config.getMinCoverSize())
    );
    viewController.txtMaxCoverSize.setText(
      String.valueOf(config.getMaxCoverSize())
    );
    ...
    viewController.txtSearchTimeout.setText(
      String.valueOf(config.getSearchTimeout())
    );
    viewController.chkGeneralManualImageSelection.setSelected(
      config.isGeneralManualImageSelection()
    );
    viewController.chkGeneralAutoLastAudio.setSelected(
      config.isGeneralAutoLastAudio()
    );
    viewController.chkGeneralPrimarySingleCover.setSelected(
      config.isPrimarySingleCover()
    );

    viewController.txtGoogleSearchPattern.setText(
      config.getGoogleSearchPattern()
    );
    viewController.chkUseCustomSearxInstance.setSelected(
      config.isSearxCustomInstanceActive()
    );
    viewController.txtCustomSearxInstance.setText(
      config.getSearxCustomInstance()
    );

    viewController.chkDiscogsUseYear.setSelected(config.isDiscogsUseYear());
    viewController.chkDiscogsUseCountry.setSelected(
      config.isDiscogsUseCountry()
    );
    viewController.txtDiscogsCountry.setText(config.getDiscogsCountry());

    viewController.txtLocalAdditionalFolderPath.setText(
      config.getLocalAdditionalFolderPath()
    );
    viewController.txtLocalNamePattern.setText(config.getLocalNamePattern());
    viewController.chkLocalScanAudiofiles.setSelected(
      config.isLocalScanAudioFiles()
    );
    viewController.chkLocalMatchAlbum.setSelected(config.isLocalMatchAlbum());
    viewController.chkLocalMatchAlbumArtist.setSelected(
      config.isLocalMatchAlbumArtist()
    );
    ...
    List<ImageView> searchEnginePictures = ImageUtil.createSearchEnginePictures(
      35,
      true,
      searchEngineLayout,
      true
  ...
  private void save() {
    ...
    appConfig.setOverwriteOnlyHigher(
      viewController.chkOverwriteOnlyHigher.isSelected()
    );
    appConfig.setMinCoverSize(
      Integer.parseInt(viewController.txtMinCoverSize.getText())
    );
    appConfig.setMaxCoverSize(
      Integer.parseInt(viewController.txtMaxCoverSize.getText())
    );
    appConfig.setSearchEngineList(
      ConfigUtil.imageToSearchEngine(
        viewController.flowPaneSettingsSearchMachineOrder.getChildren()
      )
    );
    int selectedIndex = viewController.cmbLanguage
      .getSelectionModel()
      .getSelectedIndex();
    appConfig.setLanguage(
      Language.values()[selectedIndex == -1
        ? appConfig.getLanguage().ordinal()
        : selectedIndex]
    );
    appConfig.setSearchTimeout(
      Integer.parseInt(viewController.txtSearchTimeout.getText())
    );
    appConfig.setGeneralManualImageSelection(
      viewController.chkGeneralManualImageSelection.isSelected()
    );
    appConfig.setGeneralAutoLastAudio(
      viewController.chkGeneralAutoLastAudio.isSelected()
    );
    appConfig.setPrimarySingleCover(
      viewController.chkGeneralPrimarySingleCover.isSelected()
    );

    appConfig.setGoogleSearchPattern(
      viewController.txtGoogleSearchPattern.getText()
    );
    appConfig.setSearxCustomInstanceActive(
      viewController.chkUseCustomSearxInstance.isSelected()
    );
    appConfig.setSearxCustomInstance(
      viewController.txtCustomSearxInstance.getText()
    );

    appConfig.setDiscogsUseYear(viewController.chkDiscogsUseYear.isSelected());
    appConfig.setDiscogsUseCountry(
      viewController.chkDiscogsUseCountry.isSelected()
    );
    appConfig.setDiscogsCountry(viewController.txtDiscogsCountry.getText());

    appConfig.setLocalAdditionalFolderPath(
      viewController.txtLocalAdditionalFolderPath.getText()
    );
    appConfig.setLocalNamePattern(viewController.txtLocalNamePattern.getText());
    appConfig.setLocalScanAudioFiles(
      viewController.chkLocalScanAudiofiles.isSelected()
    );
    appConfig.setLocalMatchAlbum(
      viewController.chkLocalMatchAlbum.isSelected()
    );
    appConfig.setLocalMatchAlbumArtist(
      viewController.chkLocalMatchAlbumArtist.isSelected()
    );
  ...
  private void checkForRestart() {
    Language fromCombo = getSettingsViewController()
