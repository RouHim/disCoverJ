...
public class MainService implements Service {
  ...
  public void prepareUIBindings() {
    ...
    ListenerStateProvider.getInstance().setSettingsSavedListener(
      SystemUtil::setProxy
    );

    //actions of audio detail
    InitialService initialService = ServiceLocator.get(InitialService.class);
    viewController.btnRemoveCover.setOnAction(event ->
      initialService.removeLastSelectedCover()
    );
    viewController.btnCopyCoverToClipBrd.setOnAction(event ->
      initialService.copyCoverToClipBrd()
    );
    viewController.btnPasteCoverFromClipBrd.setOnAction(event ->
      initialService.pasteCoverFromClipBrd()
    );
    viewController.btnOpenGoogleImageSearch.setOnAction(event ->
      initialService.searchOnGoogleImages()
    );
    viewController.btnFindFolder.setOnAction(event ->
      initialService.findFolder()
    );
    viewController.imgCurrentCover.setOnMouseClicked(event ->
      initialService.showCurrentCoverDetailed()
    );
  ...
  private void reportBug() {
    ...
    TextFlow txtIntro = new TextFlow(
      new Text(String.format(msg, logFile.getAbsolutePath()))
    );
